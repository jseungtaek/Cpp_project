...
void StartGame() {
  ...
  InitWindow();
  resetStage();
  if (stage == 0) setStageOne();
  if (stage == 1) setStageTwo();
  if (stage == 2) setStageThree();
  if (stage == 3) setStageFour();
  setSnake();
	setGrowthItem();
	setPosionItem();
	setGate();
...
void PlayGame() {
  while(true) {
		...
		if (num_growth == 0) setGrowthItem();
		if (num_poison == 0) setPosionItem();
		//gate
		if (num_gate == 0) setGate();
		timer();
		moveSnake();
		//drawCount();
    drawWindow();
...
void setStageTwo() {
	for (int i = 0; i<=MAP_SIZE; i++) {
		for (int j = 0; j<=MAP_SIZE; j++) {
			if (i == 0 || j == 0 || i == MAP_SIZE || j == MAP_SIZE) snakeMap[i][j] = WALL;
			else if (i %2 == 0 && j == MAP_SIZE/2) snakeMap[i][j] = WALL;
			else snakeMap[i][j] = EMPTY;
		}
	}
	snakeMap[0][0] = IMMUNE_WALL;
  snakeMap[MAP_SIZE][MAP_SIZE] = IMMUNE_WALL;
...
void setStageThree() {
	for (int i = 0; i<=MAP_SIZE; i++) {
		for (int j = 0; j<=MAP_SIZE; j++) {
			if (i == 0 || j == 0 || i == MAP_SIZE || j == MAP_SIZE) snakeMap[i][j] = WALL;
			else if (i %MAP_SIZE < MAP_SIZE/2 -4 && j == MAP_SIZE/2) snakeMap[i][j] = WALL;
			else if (i %MAP_SIZE > MAP_SIZE/2 +4 && j == MAP_SIZE/2) snakeMap[i][j] = WALL;
			else if (j %MAP_SIZE < MAP_SIZE/2 -4 && i == MAP_SIZE/2) snakeMap[i][j] = WALL;
			else if (j %MAP_SIZE > MAP_SIZE/2 +4 && i == MAP_SIZE/2) snakeMap[i][j] = WALL;
			else snakeMap[i][j] = EMPTY;
		}
	}
...
void moveSnake() {
  ...
  if (direction == 'l') snake[0].x--;
	...
	// 벽에 부딪힐 때
	if (snakeMap[snake[0].y][snake[0].x] == WALL
		|| snakeMap[snake[0].y][snake[0].x] == IMMUNE_WALL) gameover = true;
...
void setGate(){
	...
	while(1){
		k = rand() % (MAP_SIZE+1) + 1;
		s = rand() % (MAP_SIZE+1) + 1;
		if (snakeMap[k][s] == WALL && snakeMap[k][s] != GATE){
			snakeMap[k][s] = GATE;
			gate_position2.x = k;
			gate_position2.y = s;
			num_gate++;
